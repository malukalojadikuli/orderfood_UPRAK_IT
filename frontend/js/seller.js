...
function renderOrders(filteredOrders) {
    ...
    list.innerHTML = filteredOrders.map(order => createOrderCard(order)).join('');
}
...
async function toggleSoldOut(menuId) {
    try {
        ...
        showToast(result.stock === 0 ? `${item.name} sold out` : `${item.name} di-restock!`);
        renderMenu();
    } catch (error) {
...
function loadIncome() {
    ...
    const total = doneOrders.reduce((sum, o) => sum + o.total, 0);
    ...
    list.innerHTML = doneOrders.map(order => `
        <div class="income-order-card">
            ...
            <div class="income-order-amount">+ Rp ${formatPrice(order.total)}</div>
        </div>
    `).join('');
}
